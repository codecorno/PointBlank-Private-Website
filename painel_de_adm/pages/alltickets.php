		...
		<div class="col-sm-9 mailbox-right">
			<div class="mail-env">
				<table class="table mail-table" style="font-size: 13px;">
					<thead>
						<tr>
							...
							<th colspan="4" class="col-header-options">
								<div class="mail-select-options">Mostrando de <strong><?php echo $inicio+1; ?> até <?php if($pc*12 > $total){echo $total;}else{echo $pc*12;} ?></strong>
								de <strong><?php echo $total; ?></strong> Tickets</div>
					...
					<?php
						for($b = 0; $b < 12; $b++){
							if ($rank->tickets($inicio)[$b]['status'] == 0){
								echo "<tr class='unread'>";
								echo "<td style='padding: 0 0 1px 35px;'><span class='label label-red'>Pendente</span></td>";
								echo "<td class='col-name'><a href='?pg=responder&id=".$rank->tickets($inicio)[$b]['id']."' class='col-name' style='display: block;max-width: 80px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;'>".$rank->tickets($inicio)[$b]['titulo']."</a></td>";
								echo "<td class='col-subject'><a style='display: block;max-width: 255px;max-height: 16px;' href='?pg=responder&id=".$rank->tickets($inicio)[$b]['id']."' class='col-name'>".$rank->tickets($inicio)[$b]['mensagem']."</a></td>";
							}else{
								echo "<tr>";
								echo "<td style='padding: 0 0 1px 29px;'><span class='label label-secondary'>Respondido</span></td>";
								echo "<td class='col-name'><a href='?pg=lerticket&id=".$rank->tickets($inicio)[$b]['id']."' class='col-name' style='display: block;max-width: 80px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;'>".$rank->tickets($inicio)[$b]['titulo']."</a></td>";
								echo "<td class='col-subject'><a style='display: block;max-width: 255px;max-height: 16px;' href='?pg=lerticket&id=".$rank->tickets($inicio)[$b]['id']."' class='col-name'>".$rank->tickets($inicio)[$b]['mensagem']."</a></td>";
							}
							echo "<td class='col-options hidden-sm hidden-xs'></td>";
							echo "<td style='width:22%; text-align:center;'>".$rank->tickets($inicio)[$b]['nickname']."</td>";
