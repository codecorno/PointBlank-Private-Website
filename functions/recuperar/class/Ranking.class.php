...
class Ranking {
    ...
    function TotalPaginas(){
        try{
            $rank =  pg_query("SELECT * FROM accounts WHERE rank<'53' AND player_name<>''");
    ...
    function TotalPaginasClan(){
        try{
            $rank =  pg_query("SELECT * FROM clan_data ");
            $total = pg_num_rows($rank);
            $tp = $total / $this->quantidade_pag;
            return $tp;
        }catch(PDOException $e){
    ...
    function RankingGeral($inicio){
        try{
            $rank = pg_query("SELECT * FROM accounts WHERE rank < '53' AND player_name<>'' ORDER BY exp DESC LIMIT 20 OFFSET '$inicio'");
    ...
    function RankingGeralClan($inicio){
        try{
            $rank1 = pg_query("SELECT * FROM clan_data ORDER BY clan_exp DESC LIMIT 20 OFFSET '$inicio'");
	...
	function tickets($inicio){
        try{
            $rank1 = pg_query("SELECT * FROM suporte WHERE nickname='$_SESSION[username]' ORDER BY status DESC, id DESC LIMIT 15 OFFSET '$inicio'");
            $ranking1 = pg_fetch_all($rank1);
            return $ranking1;
        }catch(PDOException $e){
            echo "Erro: ".$e->getMessage();
	...
	function statusticket($numero1){
		if ($numero1 == 0){
			...
			echo "<span style='color:#FF8605;'>Respondido</span>";
		}
	}
	...
	function tipodeconta($numero){
		if ($numero == 0)
			...
			echo "Administrador   ";
		    echo "(<a href='painel_de_adm/' style='text-decoration:none;color:red;'>Painel ADM</a>)";
	}
