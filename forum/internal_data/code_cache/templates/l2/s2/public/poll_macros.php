...
'vote' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['poll']['max_votes'] != 1) {
		$__compilerTemp3 .= '
						<li>' . 'Múltiplos votos são permitidos.' . '</li>
					';
	}
	$__compilerTemp3 .= '
					';
	if ($__vars['poll']['public_votes']) {
		$__compilerTemp3 .= '
						<li>' . 'Seu voto será publicamente visível.' . '</li>
					';
	}
	$__compilerTemp3 .= '
				';
	...
	<div class="block-footer">
		<span class="block-footer-controls">
			' . $__templater->button('Votar', array(
		'type' => 'submit',
	...
	if ($__templater->method($__vars['poll'], 'canViewResults', array())) {
		$__finalCompiled .= '
				' . $__templater->button('Ver resultados', array(
...
'result' => function($__templater, array $__arguments, array $__vars)
{
				...
				if ($__vars['canShowVoters']) {
					$__finalCompiled .= '
									<a class="fauxBlockLink-blockLink"
										...
										role="button"
										tabindex="0">
										<span class="u-muted">' . 'Votos' . $__vars['xf']['language']['label_separator'] . '</span> ' . $__templater->filter($__vars['response']['response_vote_count'], array(array('number', array()),), true) . '
									</a>
								';
				} else {
					$__finalCompiled .= '
	...
	if ($__vars['showFooter']) {
		...
		if ($__vars['simpleDisplay']) {
			$__compilerTemp1 .= '
					<a href="' . $__templater->escape($__templater->method($__vars['poll'], 'getLink', array('content', ))) . '">' . 'Ver comentários' . $__vars['xf']['language']['ellipsis'] . '</a>
				';
		} else {
			$__compilerTemp1 .= '
					';
...
'add_edit_inputs' => function($__templater, array $__arguments, array $__vars)
{
			...
			foreach ($__vars['poll']['Responses'] AS $__vars['response']) {
				...
				if ($__templater->method($__vars['poll'], 'canEditDetails', array())) {
					$__compilerTemp1 .= '
						...
						'placeholder' => 'Opção' . $__vars['xf']['language']['ellipsis'],
					)) . '
						';
				} else {
					$__compilerTemp1 .= '
	...
	$__finalCompiled .= $__templater->formRadioRow(array(
		...
		'label' => 'Máximo de respostas selecionáveis',
		'explain' => 'Este é o número máximo de respostas que um eleitor pode selecionar ao votar.',
	)) . '
	...
	$__compilerTemp4 = array(array(
		...
		'label' => '
			' . 'Permitir que os eleitores mudem seus votos' . '
		',
		'_type' => 'option',
	));
	...
	$__compilerTemp4[] = array(
		...
		'label' => '
			' . 'Permitir que os resultados sejam visualizados sem voto' . '
		',
		'_type' => 'option',
	);
	if ($__vars['poll'] AND $__vars['poll']['close_date']) {
		...
		$__compilerTemp4[] = array(
			...
			'label' => 'Fechar esta enquete após' . $__vars['xf']['language']['label_separator'],
			'_dependent' => array('
...
'delete_block' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['poll']['voter_count']) {
		$__compilerTemp1[] = array(
			'value' => 'reset',
			'label' => 'Resetar todos os votos',
			'_type' => 'option',
		);
	}
	$__compilerTemp1[] = array(
		'value' => 'remove',
		'label' => 'Excluir enquete completa',
		'_type' => 'option',
	);
	$__finalCompiled .= $__templater->form('
		<div class="block-container">
			<div class="block-body">
				<h2 class="block-header">' . 'Excluir enquete' . '</h2>
