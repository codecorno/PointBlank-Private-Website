	...
	if (strlen(trim($__compilerTemp3)) > 0) {
		$__compilerTemp2 .= '
												<div class="buttonGroup-buttonWrapper">
													' . $__templater->button('Ferramentas de moderação', array(
			...
			'aria-haspopup' => 'true',
													...
													<div class="menu" data-menu="menu" aria-hidden="true">
														<div class="menu-content">
															<h3 class="menu-header">' . 'Ferramentas de moderação' . '</h3>
															' . $__compilerTemp3 . '
														</div>
													</div>
												</div>
	...
	if ($__templater->method($__vars['user'], 'canViewPostsOnProfile', array())) {
		$__finalCompiled .= '
					<a href="' . $__templater->func('link', array('members', $__vars['user'], ), true) . '"
						...
						role="tab"
						aria-controls="profile-posts">' . 'Posts do perfil' . '</a>
				';
	}
	$__finalCompiled .= '
...
<ul class="tabPanes js-memberTabPanes">
	...
	if ($__templater->method($__vars['user'], 'canViewPostsOnProfile', array())) {
		...
		if (!$__templater->test($__vars['profilePosts'], 'empty', array())) {
			...
			$__finalCompiled .= '
							<div class="block-row js-replyNoMessages">' . 'Não há mensagens no perfil de ' . $__templater->escape($__vars['user']['username']) . ' ainda.' . '</div>
						';
		}
		$__finalCompiled .= '
					</div>
	...
	if ($__templater->method($__vars['user'], 'canViewLatestActivity', array())) {
		...
		<li data-href="' . $__templater->func('link', array('members/latest-activity', $__vars['user'], ), true) . '" role="tabpanel" aria-labelledby="latest-activity">
			<div class="blockMessage">' . 'Carregando' . $__vars['xf']['language']['ellipsis'] . '</div>
		</li>
	';
	}
	$__finalCompiled .= '
