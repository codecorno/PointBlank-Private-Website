...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Visitantes atuais');
...
<div class="block">
	<div class="block-container">
		<h2 class="block-tabHeader tabs hScroller" data-xf-init="h-scroller">
			<span class="hScroller-scroll">
				' . '
				<a href="' . $__templater->func('link', array('online', ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == '') ? 'is-active' : '') . '">' . 'Todos' . '</a>
				<a href="' . $__templater->func('link', array('online', null, array('type' => 'member', ), ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == 'member') ? 'is-active' : '') . '">' . 'Membros' . '</a>
				<a href="' . $__templater->func('link', array('online', null, array('type' => 'guest', ), ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == 'guest') ? 'is-active' : '') . '">' . 'Visitantes' . '</a>
				<a href="' . $__templater->func('link', array('online', null, array('type' => 'robot', ), ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == 'robot') ? 'is-active' : '') . '">' . 'Robôs' . '</a>
				' . '
			</span>
		</h2>
		';
	if (!$__templater->test($__vars['activities'], 'empty', array())) {
		...
		if ($__templater->isTraversable($__vars['activities'])) {
			foreach ($__vars['activities'] AS $__vars['activity']) {
				...
				if ($__vars['activity']['User']) {
					...
					$__finalCompiled .= '
									<h3 class="contentRow-header">' . 'Visitante' . '</h3>
								';
				}
				$__finalCompiled .= '
				...
				if ((!$__vars['activity']['User']) OR $__templater->method($__vars['activity']['User'], 'canViewCurrentActivity', array())) {
					...
					if ($__vars['activity']['description']) {
						...
						$__finalCompiled .= '
												<span title="' . $__templater->escape($__vars['activity']['controller_name']) . '::' . $__templater->escape($__vars['activity']['controller_action']) . '">' . 'Visualizando página desconhecida' . '</span>
											';
					} else {
						$__finalCompiled .= '
												' . 'Visualizando página desconhecida' . '
											';
					}
					$__finalCompiled .= '
											</li>
				...
				if (($__vars['activity']['view_state'] == 'error') AND $__templater->method($__vars['xf']['visitor'], 'canBypassUserPrivacy', array())) {
					$__finalCompiled .= '
						...
						'title' => $__templater->filter('Visualizando um erro', array(array('for_attr', array()),), false),
					)) . '
											<span class="u-srOnly">' . 'Visualizando um erro' . '</span>
											</li>
										';
				}
				$__finalCompiled .= '
		...
		$__finalCompiled .= '
			<div class="block-row">' . 'Nenhum resultado encontrado.' . '</div>
		';
	}
	$__finalCompiled .= '
	</div>
