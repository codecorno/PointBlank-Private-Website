		...
		if ($__vars['showFooter']) {
			$__finalCompiled .= '
				<div class="memberOverviewBlock-seeMore">
					<a href="' . $__templater->func('link', array('members', null, array('key' => $__vars['memberStat']['member_stat_key'], ), ), true) . '">' . 'Ver mais' . $__vars['xf']['language']['ellipsis'] . '</a>
				</div>
			';
		}
		$__finalCompiled .= '
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['active'] ? $__templater->escape($__vars['active']['title']) : 'Membros notáveis'));
	$__finalCompiled .= '
...
';
	if ($__vars['userNotFound']) {
		$__finalCompiled .= '
	<div class="blockMessage blockMessage--error blockMessage--iconic">' . 'O membro especificado não pode ser encontrado. Insira o nome completo de um membro.' . '</div>
';
	}
	$__finalCompiled .= '
...
';
	if ($__templater->test($__vars['memberStats'], 'empty', array())) {
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Nenhum membro notável pode ser mostrado atualmente.' . '</div>
';
	} else {
		$__finalCompiled .= '
	<section class="block">
		...
		if ($__vars['active']) {
			...
			if ($__compilerTemp2) {
				$__finalCompiled .= '
						<li class="block-row">' . 'Nenhum usuário corresponde aos critérios especificados.' . '</li>
					';
			}
			$__finalCompiled .= '
				</ol>
