...
return array('macros' => array('attribution' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['showTargetUser'] AND ($__vars['profilePost']['user_id'] != $__vars['profilePost']['profile_user_id'])) {
		...
		<span class="u-srOnly">' . '' . ($__templater->escape($__vars['profilePost']['User']['username']) ?: $__templater->escape($__vars['profilePost']['username'])) . ' escreveu no perfil de ' . ($__templater->escape($__vars['profilePost']['ProfileUser']['username']) ?: 'Unknown') . '.' . '</span>
	';
	} else {
		$__finalCompiled .= '
		' . $__templater->func('username_link', array($__vars['profilePost']['User'], true, array(
...
'profile_post' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['profilePost']['message_state'] == 'deleted') {
		...
		$__finalCompiled .= '
							<div class="messageNotice messageNotice--moderated">
								' . 'Esta mensagem está aguardando a aprovação do moderador e é invisível para os visitantes normais.' . '
							</div>
						';
	}
	$__finalCompiled .= '
	...
	if ($__templater->method($__vars['profilePost'], 'isIgnored', array())) {
		$__finalCompiled .= '
							<div class="messageNotice messageNotice--ignored">
								' . 'Você está ignorando o conteúdo desse membro.' . '
							</div>
						';
	}
	$__finalCompiled .= '
	...
	if ($__vars['allowInlineMod'] AND $__templater->method($__vars['profilePost'], 'canUseInlineModeration', array())) {
		$__compilerTemp2 .= '
			...
			'label' => 'Selecione para moderação',
			'hiddenlabel' => 'true',
			'_type' => 'option',
		))) . '
										</span>
	...
	if ($__templater->method($__vars['profilePost'], 'canEdit', array())) {
		...
		$__compilerTemp2 .= '
										<a href="' . $__templater->func('link', array('profile-posts/edit', $__vars['profilePost'], ), true) . '"
											...
											data-menu-closer="true">' . 'Editar' . '</a>
										';
	...
	if ($__templater->method($__vars['profilePost'], 'canDelete', array('soft', ))) {
		$__compilerTemp2 .= '
										<a href="' . $__templater->func('link', array('profile-posts/delete', $__vars['profilePost'], ), true) . '"
											...
											data-xf-click="overlay">' . 'Excluir' . '</a>
										';
		$__vars['hasActionBarMenu'] = true;
		$__compilerTemp2 .= '
									';
	...
	if ($__templater->method($__vars['profilePost'], 'canWarn', array())) {
		$__compilerTemp2 .= '
										<a href="' . $__templater->func('link', array('profile-posts/warn', $__vars['profilePost'], ), true) . '"
											class="actionBar-action actionBar-action--warn actionBar-action--menuItem">' . 'Alertar' . '</a>
										';
		$__vars['hasActionBarMenu'] = true;
		$__compilerTemp2 .= '
									';
	...
	if ($__vars['hasActionBarMenu']) {
		$__compilerTemp2 .= '
										<a class="actionBar-action actionBar-action--menuTrigger"
											data-xf-click="menu"
											title="' . $__templater->filter('Mais opções', array(array('for_attr', array()),), true) . '"
											role="button"
											tabindex="0"
											aria-expanded="false"
	...
	if ($__templater->method($__vars['profilePost'], 'canComment', array())) {
		...
		$__finalCompiled .= $__templater->form('
			...
			'minHeight' => '40',
			'placeholder' => 'Escreva um comentário' . $__vars['xf']['language']['ellipsis'],
			'submitText' => 'Postar comentário',
			'deferred' => true,
			'simpleSubmit' => true,
		), $__vars) . '
													</div>
													<div class="editorPlaceholder-placeholder">
														<div class="input"><span class="u-muted"> ' . 'Escreva um comentário' . $__vars['xf']['language']['ellipsis'] . '</span></div>
													</div>
												</div>
											</div>
										</div>
...
'profile_post_deleted' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="message message--simple' . ($__templater->method($__vars['profilePost'], 'isIgnored', array()) ? ' is-ignored' : '') . ' js-inlineModContainer"
		...
		'log' => $__vars['profilePost']['DeletionLog'],
					...
					<a href="' . $__templater->func('link', array('profile-posts/show', $__vars['profilePost'], ), true) . '" class="u-jsOnly" data-xf-click="inserter" data-replace="[data-content=profile-post-' . $__templater->escape($__vars['profilePost']['profile_post_id']) . ']">' . 'Mostrar' . $__vars['xf']['language']['ellipsis'] . '</a>
...
'profile_post_simple' => function($__templater, array $__arguments, array $__vars)
{
	...
	$__finalCompiled .= '
			...
			<div class="contentRow-minor">
				...
				<a href="' . $__templater->func('link', array('profile-posts', $__vars['profilePost'], ), true) . '" rel="nofollow" class="contentRow-extra" data-xf-click="overlay" data-xf-init="tooltip" title="' . $__templater->filter('Interagir', array(array('for_attr', array()),), true) . '">&#8226;&#8226;&#8226;</a>
			</div>
		</div>
	</div>
';
		...
		if ($__vars['comment']['message_state'] == 'moderated') {
			$__finalCompiled .= '
											<a href="' . $__templater->func('link', array('profile-posts/comments/approve', $__vars['comment'], array('t' => $__templater->func('csrf_token', array(), false), ), ), true) . '"
												class="actionBar-action actionBar-action--approve actionBar-action--menuItem">' . 'Aprovar' . '</a>
											';
			$__vars['hasActionBarMenu'] = true;
			$__finalCompiled .= '
										';
			...
			$__finalCompiled .= '
											<a href="' . $__templater->func('link', array('profile-posts/comments/unapprove', $__vars['comment'], array('t' => $__templater->func('csrf_token', array(), false), ), ), true) . '"
												class="actionBar-action actionBar-action--unapprove actionBar-action--menuItem">' . 'Desaprovar' . '</a>
											';
			$__vars['hasActionBarMenu'] = true;
			$__finalCompiled .= '
										';
	...
	<div class="message-responseRow">
		...
		'log' => $__vars['comment']['DeletionLog'],
							...
							<a href="' . $__templater->func('link', array('profile-posts/comments/show', $__vars['comment'], ), true) . '" class="u-jsOnly"
								...
								data-replace="[data-content=profile-post-comment-' . $__templater->escape($__vars['comment']['profile_post_comment_id']) . ']">' . 'Mostrar' . $__vars['xf']['language']['ellipsis'] . '</a>
						</div>
					</div>
...
'quick_post' => function($__templater, array $__arguments, array $__vars)
{
	...
	' . $__templater->form('
		...
		'simple' => true,
		'placeholder' => (($__vars['xf']['visitor']['user_id'] == $__vars['user']['user_id']) ? 'Atualizar seu status' . $__vars['xf']['language']['ellipsis'] : 'Escreva algo' . $__vars['xf']['language']['ellipsis']),
		'submitText' => 'Post',
...
'submit' => function($__templater, array $__arguments, array $__vars)
{
	...
	';
	$__vars['placeholder'] = (($__vars['xf']['visitor']['user_id'] == $__vars['user']['user_id']) ? 'Atualizar seu status' . $__vars['xf']['language']['ellipsis'] : 'Escreva algo' . $__vars['xf']['language']['ellipsis']);
	$__finalCompiled .= '
