...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Fóruns seguidos');
	$__finalCompiled .= '
			...
			foreach ($__compilerTemp2 AS $__vars['id'] => $__vars['treeEntry']) {
				...
				if ($__vars['forumWatch']) {
					...
					if ($__vars['forumWatch']['notify_on'] == 'thread') {
						...
						$__compilerTemp3 .= '
									<li>' . 'Novas mensagens' . '</li>
								';
					}
					$__compilerTemp4 = '';
					if ($__vars['forumWatch']['send_alert']) {
						$__compilerTemp4 .= '<li>' . 'Alertas' . '</li>';
					}
					$__compilerTemp5 = '';
					if ($__vars['forumWatch']['send_email']) {
						$__compilerTemp5 .= '<li>' . 'E-mails' . '</li>';
					}
			...
			'aria-haspopup' => 'true',
				...
				<div class="menu" data-menu="menu" aria-hidden="true">
					<div class="menu-content">
						...
						' . '
						<a href="' . $__templater->func('link', array('watched/forums/manage', null, array('state' => 'watch_no_email', ), ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Desativar notificação por e-mail' . '</a>
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Você não está seguindo nenhum fórum.' . '</div>
';
	}
