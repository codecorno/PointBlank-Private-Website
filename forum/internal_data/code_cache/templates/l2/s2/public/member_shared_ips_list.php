...
return array('macros' => array('shared_block' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['shared']) {
				...
				if ($__templater->isTraversable($__vars['share']['ips'])) {
					foreach ($__vars['share']['ips'] AS $__vars['ip']) {
						...
						if ($__vars['ip']['total'] > 1) {
							$__finalCompiled .= '
															<li>' . '' . $__templater->filter($__vars['ip']['total'], array(array('number', array()),), true) . ' vezes' . '</li>
		...
		$__finalCompiled .= '
				<div class="block-body block-row">' . 'Nenhum usuário correspondente foi encontrado.' . '</div>
			';
	}
	$__finalCompiled .= '
		</div>
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('IPs compartilhados para ' . $__templater->escape($__vars['user']['username']) . '');
	$__finalCompiled .= '
