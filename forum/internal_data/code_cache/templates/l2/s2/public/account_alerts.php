...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Alertas');
	$__finalCompiled .= '
...
<div class="block">
	...
	if (!$__templater->test($__vars['alerts'], 'empty', array())) {
		...
		$__finalCompiled .= '
				<div class="block-row">' . 'Você não tem alertas recentes.' . '</div>
			';
	} else {
		$__finalCompiled .= '
				<div class="block-row">' . 'Nenhum alerta pode ser exibido. Selecione uma página diferente.' . '</div>
			';
	}
	$__finalCompiled .= '
		</div>
