...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['isStarred'] ? 'Desmarcar' : 'Star'));
	$__finalCompiled .= '
...
';
	...
	if ($__vars['isStarred']) {
		$__compilerTemp1 .= '
					' . 'Confirme que deseja desmarcar a seguinte conversa' . $__vars['xf']['language']['label_separator'] . '
				';
	} else {
		$__compilerTemp1 .= '
					' . 'Confirme que você deseja marcar com estrela a seguinte conversa' . $__vars['xf']['language']['label_separator'] . '
				';
	}
	...
	<div class="block-container">
		...
		'submit' => ($__vars['isStarred'] ? 'Desmarcar' : 'Star'),
	), array(
		'rowtype' => 'simple',
	)) . '
	</div>
