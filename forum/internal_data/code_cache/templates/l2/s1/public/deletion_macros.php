...
return array('macros' => array('notice' => function($__templater, array $__arguments, array $__vars)
{
	...
	$__finalCompiled .= '
		<li>' . 'Excluído por ' . ($__templater->escape($__vars['log']['delete_username']) ?: 'N/A') . '' . '</li>
		';
	if ($__vars['log']) {
		$__finalCompiled .= '
		...
		if ($__vars['log']['delete_reason']) {
			$__finalCompiled .= '
				<li>' . 'Motivo' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['log']['delete_reason']) . '</li>
			';
		}
		$__finalCompiled .= '
		';
