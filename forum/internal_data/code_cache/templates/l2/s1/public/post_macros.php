...
return array('macros' => array('post' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__templater->method($__vars['post'], 'isUnread', array())) {
		$__finalCompiled .= '
								<li><span class="message-newIndicator">' . 'Novo' . '</span></li>
							';
	}
	$__finalCompiled .= '
							<li>
	...
	if ($__vars['post']['last_edit_date']) {
		...
		if ($__vars['post']['user_id'] == $__vars['post']['last_edit_user_id']) {
			$__finalCompiled .= '
									' . 'Última edição' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['post']['last_edit_date'], array(
			))) . '
								';
		} else {
			$__finalCompiled .= '
									' . 'Última edição por um moderador' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['post']['last_edit_date'], array(
			))) . '
								';
		}
		$__finalCompiled .= '
	...
	if ($__templater->method($__vars['thread'], 'canReply', array())) {
		...
		if ($__vars['xf']['options']['multiQuote']) {
			$__compilerTemp3 .= '
													<a href="' . $__templater->escape($__vars['quoteLink']) . '"
														...
														title="' . $__templater->filter('Alternar várias citações', array(array('for_attr', array()),), true) . '"
		...
		$__compilerTemp3 .= '
												...
												<a href="' . $__templater->escape($__vars['quoteLink']) . '"
													...
													data-xf-click="quote"
													data-quote-href="' . $__templater->func('link', array('posts/quote', $__vars['post'], ), true) . '">' . 'Responder' . '</a>
											';
	}
	$__compilerTemp3 .= '
										';
	...
	if ($__vars['post']['edit_count'] AND $__templater->method($__vars['post'], 'canViewHistory', array())) {
		$__compilerTemp4 .= '
												<a href="' . $__templater->func('link', array('posts/history', $__vars['post'], ), true) . '"
													...
													data-menu-closer="true">' . 'Histórico' . '</a>
...
'post_deleted' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="message message--deleted message--post' . ($__templater->method($__vars['post'], 'isIgnored', array()) ? ' is-ignored' : '') . ($__templater->method($__vars['post'], 'isUnread', array()) ? ' is-unread' : '') . ' js-post js-inlineModContainer"
		...
		'log' => $__vars['post']['DeletionLog'],
					...
					<a href="' . $__templater->func('link', array('posts/show', $__vars['post'], ), true) . '" class="u-jsOnly" data-xf-click="inserter" data-replace="[data-content=post-' . $__templater->escape($__vars['post']['post_id']) . ']">' . 'Mostrar' . $__vars['xf']['language']['ellipsis'] . '</a>
