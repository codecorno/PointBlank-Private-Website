	...
	if ($__vars['thread']['discussion_state'] == 'deleted') {
		...
		$__compilerTemp1 .= '
						<dd class="blockStatus-message blockStatus-message--moderated">
							' . 'Aguardando aprovação antes de ser exibido publicamente.' . '
						</dd>
					';
	}
	$__compilerTemp1 .= '
...
';
	...
	<ul class="listInline listInline--bullet">
		...
		'title' => $__templater->filter('Criador do tópico', array(array('for_attr', array()),), false),
	)) . '
...
<div class="block block--messages" data-xf-init="' . ($__vars['canInlineMod'] ? 'inline-mod' : '') . '" data-type="post" data-href="' . $__templater->func('link', array('inline-mod', ), true) . '">
	...
	if ($__templater->method($__vars['thread'], 'canEdit', array())) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->func('link', array('threads/edit', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Editar tópico' . '</a>
										';
	}
	$__compilerTemp7 .= '
										';
	...
	if ($__templater->method($__vars['thread'], 'canCreatePoll', array())) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->func('link', array('threads/poll/create', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Criar enquete' . '</a>
										';
	}
	$__compilerTemp7 .= '
										';
	if ($__templater->method($__vars['thread'], 'canDelete', array('soft', ))) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->func('link', array('threads/delete', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Excluir tópico' . '</a>
										';
	}
	$__compilerTemp7 .= '
										';
	if ($__templater->method($__vars['thread'], 'canMove', array())) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->func('link', array('threads/move', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Mover tópico' . '</a>
										';
	}
	$__compilerTemp7 .= '
										';
...
<div class="blockMessage blockMessage--none">
	' . $__templater->callMacro('share_page_macros', 'buttons', array(
		'iconic' => true,
		'label' => 'Compartilhar' . $__vars['xf']['language']['label_separator'],
	), $__vars) . '
</div>
