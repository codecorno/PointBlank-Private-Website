...
'results' => function($__templater, array $__arguments, array $__vars)
{
	...
	';
		...
		if ($__vars['xf']['visitor']['user_id'] AND ($__vars['findNew']['filters']['unread'] AND ($__templater->func('count', array($__vars['findNew']['filters'], ), false) == 1))) {
			$__finalCompiled .= '
				' . 'Você não tem mensagens não lidas. Você pode <a href="' . $__templater->func('link', array($__vars['latestRoute'], null, array('skip' => 1, ), ), true) . '" rel="nofollow">ver todas as postagens mais recentes</a>.' . '
			';
		} else {
			$__finalCompiled .= '
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Novos posts');
...
';
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canCreateThread', array())) {
		$__compilerTemp2 .= '
		' . $__templater->button('
			' . 'Postar tópico' . $__vars['xf']['language']['ellipsis'] . '
		', array(
