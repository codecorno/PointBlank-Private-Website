...
<div class="block">
	...
	if ($__vars['user']['Profile']['birthday']['timeStamp']) {
		...
		if ($__vars['user']['Profile']['birthday']['age']) {
			$__compilerTemp3 .= '
											' . $__templater->func('parens', array(('Idade' . $__vars['xf']['language']['label_separator'] . ' ') . $__vars['user']['Profile']['birthday']['age'], ), true) . '
										';
		}
		$__compilerTemp3 .= '
									';
	...
	if ($__templater->method($__vars['user'], 'canViewIdentities', array())) {
		...
		if ($__templater->method($__vars['xf']['visitor'], 'canStartConversationWith', array($__vars['user'], ))) {
			$__compilerTemp4 .= '
								<dl class="pairs pairs--columns pairs--fixedSmall">
									<dt>' . 'Conversa' . '</dt>
									<dd><a href="' . $__templater->func('link', array('conversations/add', '', array('to' => $__vars['user']['username'], ), ), true) . '">' . 'Iniciar conversa' . '</a></dd>
								</dl>
							';
		}
		$__compilerTemp4 .= '
		...
		if (strlen(trim($__compilerTemp4)) > 0) {
			$__compilerTemp1 .= '
					<div class="block-row block-row--separated">
						<h4 class="block-textHeader">' . 'Contato' . '</h4>
						' . $__compilerTemp4 . '
					</div>
				';
		}
	...
	if ($__vars['user']['Profile']['signature']) {
		$__compilerTemp1 .= '
				<div class="block-row block-row--separated">
					<h4 class="block-textHeader">' . 'Assinatura' . '</h4>
	...
	if ($__vars['followingCount']) {
		...
		if (strlen(trim($__compilerTemp5)) > 0) {
			$__compilerTemp1 .= '
					<div class="block-row block-row--separated">
						<h4 class="block-textHeader">' . 'Seguindo' . '</h4>
						<ul class="listHeap">
							' . $__compilerTemp5 . '
						</ul>
						';
	...
	if (!$__templater->test($__vars['trophies'], 'empty', array()) AND $__vars['xf']['options']['enableTrophies']) {
		$__compilerTemp1 .= '
				<div class="block-row block-row--separated">
					<h4 class="block-textHeader">' . 'Troféus' . '</h4>
					<ol class="listPlain">
						';
