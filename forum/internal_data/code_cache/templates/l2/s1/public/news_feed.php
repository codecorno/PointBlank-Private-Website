...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Seu feed de notícias');
	$__finalCompiled .= '
...
<div class="block">
	...
	if (!$__templater->test($__vars['newsFeedItems'], 'empty', array())) {
		...
		$__finalCompiled .= '
			<div class="block-body js-newsFeedTarget">
				<div class="block-row">
					' . 'Seu feed de notícias está vazio.' . '
					<a href="' . $__templater->func('link', array('account/following', ), true) . '">' . 'Siga as pessoas para preenchê-lo.' . '</a>
				</div>
			</div>
		';
	}
