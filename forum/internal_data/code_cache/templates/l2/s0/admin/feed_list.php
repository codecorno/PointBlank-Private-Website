...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Importador de feed RSS');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Adicionar feed', array(
		...
		if ($__templater->isTraversable($__vars['feeds'])) {
			foreach ($__vars['feeds'] AS $__vars['feed']) {
				...
				array(
					...
					'submit' => 'true',
					'tooltip' => 'Ativar / desativar \'' . $__vars['feed']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				),
				array(
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Nenhum feed ainda foi registrado.' . '</div>
';
	}
