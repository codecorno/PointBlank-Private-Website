...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Promoções de grupos de usuários');
	$__finalCompiled .= '
...
';
	...
	' . $__templater->button('Adicionar promoção', array(
		...
		if ($__templater->isTraversable($__vars['userGroupPromotions'])) {
			foreach ($__vars['userGroupPromotions'] AS $__vars['userGroupPromotion']) {
				$__compilerTemp1 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Ativar / desativar \'' . $__vars['userGroupPromotion']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
