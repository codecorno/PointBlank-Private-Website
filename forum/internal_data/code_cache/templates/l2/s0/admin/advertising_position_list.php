...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Posições de publicidade');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Adicionar posição de publicidade', array(
		...
		if ($__templater->isTraversable($__vars['advertisingPositions'])) {
			foreach ($__vars['advertisingPositions'] AS $__vars['advertisingPosition']) {
				$__compilerTemp1 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Ativar / desativar \'' . $__vars['advertisingPosition']['position_id'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
