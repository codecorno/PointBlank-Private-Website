...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Atualizações de usuário expiradas' . ($__vars['userUpgrade'] ? (': ' . $__templater->escape($__vars['userUpgrade']['title'])) : ''));
	$__finalCompiled .= '
...
' . $__templater->form('
	<div class="block-container">
		...
		'ac' => 'single',
				...
				' . $__templater->button('Filtrar', array(
		'type' => 'submit',
	), '', array(
	)) . '
			', array(
	...
	<div class="block">
		...
		if ($__templater->isTraversable($__vars['expiredUpgrades'])) {
			foreach ($__vars['expiredUpgrades'] AS $__vars['expiredUpgrade']) {
				...
				array(
					'_type' => 'cell',
					'html' => ($__vars['expiredUpgrade']['end_date'] ? $__templater->func('date', array($__vars['expiredUpgrade']['end_date'], ), true) : 'Permanente'),
				))) . '
					';
			}
		}
		$__finalCompiled .= $__templater->dataList('
			...
			'_type' => 'cell',
			'html' => '<a href="' . $__templater->func('link', array('user-upgrades/expired', $__vars['userUpgrade'], array('order' => 'username', 'direction' => '', ) + $__vars['linkParams'], ), true) . '">' . 'Usuário' . '</a>',
		),
		array(
			'_type' => 'cell',
			'html' => 'Upgrade title',
		...
		array(
			'_type' => 'cell',
			'html' => '<a href="' . $__templater->func('link', array('user-upgrades/expired', $__vars['userUpgrade'], array('order' => 'end_date', 'direction' => 'desc', ) + $__vars['linkParams'], ), true) . '">' . 'Data final' . '</a>',
		))) . '
					' . $__compilerTemp1 . '
				', array(
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Nenhum resultado encontrado.' . '</div>
';
	}
