...
';
	...
	if ($__vars['positionId']) {
		$__compilerTemp1 .= '
					' . 'Confirme que deseja excluir o seguinte widget da posição \'' . $__templater->escape($__vars['positionId']) . '\'.' . '
				';
	} else {
		$__compilerTemp1 .= '
	...
	if ($__templater->func('count', array($__vars['widget']['positions'], ), false) == 1) {
		$__compilerTemp2 .= '
			...
			'name' => 'delete',
			'label' => 'Esta é a última posição para este widget. Excluir permanentemente o widget.',
			'_type' => 'option',
		)), array(
