...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('BBcodes personalizados');
	$__finalCompiled .= '
...
';
	...
	<div class="buttonGroup">
		' . $__templater->button('Adicionar BBcode', array(
			...
			foreach ($__vars['bbCodes'] AS $__vars['bbCode']) {
				...
				if ($__vars['exportView']) {
					$__compilerTemp3[] = array(
						...
						'tooltip' => ($__vars['bbCode']['addon_id'] ? 'BBcodes que pertencem a um complemento não podem ser exportados de forma independente.' : ''),
						'_type' => 'toggle',
						'html' => '',
					);
				}
				...
				if (!$__vars['exportView']) {
					$__compilerTemp3[] = array(
						...
						'submit' => 'true',
						'tooltip' => 'Ativar / desativar \'' . (('[' . $__vars['bbCode']['bb_code_id']) . ']') . '\'',
						'_type' => 'toggle',
						'html' => '',
					);
					$__compilerTemp3[] = array(
