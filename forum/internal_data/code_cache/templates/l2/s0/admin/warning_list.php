...
';
	...
	' . $__templater->button('Adicionar alerta', array(
		...
		if ($__templater->isTraversable($__vars['actions'])) {
			foreach ($__vars['actions'] AS $__vars['action']) {
				...
				if ($__vars['action']['action'] == 'ban') {
					...
					$__compilerTemp3 .= '
												<li>' . 'Adicionar aos grupos selecionados' . '</li>
											';
				} else {
					$__compilerTemp3 .= '
				...
				$__compilerTemp2 .= $__templater->dataRow(array(
					...
					'html' => '
								<div class="dataList-mainRow">
									' . 'Pontos' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['action']['points']) . '
