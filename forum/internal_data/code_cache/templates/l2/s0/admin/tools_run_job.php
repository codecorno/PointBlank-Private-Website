...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Processando' . $__vars['xf']['language']['ellipsis']);
	$__finalCompiled .= '
';
...
';
	...
	';
		...
		<form action="' . $__templater->func('link', array('tools/run-job', null, array('cancel' => $__vars['jobId'], ), ), true) . '" method="post" onsubmit="window.stop()" class="u-pullRight">
			' . $__templater->button('Cancelar', array(
			'type' => 'submit',
		), '', array(
		)) . '
...
<form action="' . $__templater->func('link', array('tools/run-job', ), true) . '" method="post" class="blockMessage" data-xf-init="auto-submit">
	...
	if ($__vars['status']) {
		...
		$__finalCompiled .= 'Processando' . $__vars['xf']['language']['ellipsis'];
	}
	$__finalCompiled .= '</div>
