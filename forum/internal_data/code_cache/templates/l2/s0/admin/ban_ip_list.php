...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Endereço de IP banido.');
	$__finalCompiled .= '
		...
		if ($__templater->isTraversable($__vars['ipBans'])) {
			foreach ($__vars['ipBans'] AS $__vars['ipBan']) {
				...
				array(
					'_type' => 'cell',
					'html' => '
								' . ($__vars['ipBan']['last_triggered_date'] ? $__templater->func('date_dynamic', array($__vars['ipBan']['last_triggered_date'], ), true) : 'Nunca') . '
							',
				),
				array(
					'_type' => 'cell',
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Nenhum IP foi banido.' . '</div>
';
	}
	$__finalCompiled .= '
