	...
	if ($__vars['depth'] == 1) {
		...
		<li class="p-nav-section' . (($__vars['navigation']['navigation_id'] == $__vars['selectedTab']) ? ' is-active' : '') . '" data-navigation-id="' . $__templater->escape($__vars['navigation']['navigation_id']) . '">
			<div class="p-nav-sectionHeader">
				...
				<a class="p-nav-sectionToggle js-navSectionToggle" role="button" tabindex="0" aria-label="' . $__templater->filter('Expandir', array(array('for_attr', array()),), true) . '"></a>
			</div>
	';
	} else if ($__vars['depth'] == 2) {
		$__finalCompiled .= '
...
<head>
	...
	<title>
		' . $__templater->func('page_title', array('%s | ' . $__vars['xf']['options']['boardTitle'] . ' - ' . 'Painel de controle de administração', $__vars['xf']['options']['boardTitle'] . ' - ' . 'Painel de controle de administração', null)) . '
	</title>
	';
...
<header class="p-header">
	<div class="p-header-buttons p-header-buttons--main">
		...
		<a href="' . $__templater->func('link', array('index', ), true) . '" class="p-header-button" aria-label="' . $__templater->filter('Inicio', array(array('for_attr', array()),), true) . '">' . $__templater->fontAwesome('fa-home', array(
	)) . '</a>
	...
	if ($__templater->method($__vars['xf']['visitor'], 'hasAdminPermission', array('option', ))) {
		$__finalCompiled .= '
			<a class="p-header-button" data-xf-key="' . $__templater->filter('o', array(array('for_attr', array()),), true) . '" data-xf-click="menu" role="button" tabindex="0"
				aria-label="' . $__templater->filter('Opções', array(array('for_attr', array()),), true) . '"
				aria-expanded="false"
				aria-haspopup="true">
	...
	$__finalCompiled .= '
		...
		<form action="' . $__templater->func('link', array('search/search', ), true) . '" class="menu menu--structural menu--veryWide" data-xf-init="admin-search" data-menu="menu" aria-hidden="true">
			<div class="menu-content">
				<h3 class="menu-header">' . 'Pesquisar' . $__vars['xf']['language']['ellipsis'] . '</h3>
				<div class="menu-row">
...
<footer class="p-footer">
	<div class="p-footer-row">
		<div class="p-footer-row-main">
			<a href="' . $__templater->func('link', array('login/logout', null, array('t' => $__templater->func('csrf_token', array(), false), ), ), true) . '">' . 'Sair' . '</a>
		</div>
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canChangeLanguage', array())) {
		$__finalCompiled .= '
				<a href="' . $__templater->func('link', array('languages/admin', ), true) . '" data-xf-click="overlay" data-xf-init="tooltip" title="' . $__templater->filter('Seletor de idioma', array(array('for_attr', array()),), true) . '">
