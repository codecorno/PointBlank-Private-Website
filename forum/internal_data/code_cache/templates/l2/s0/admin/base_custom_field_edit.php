...
},), 'code' => function($__templater, array $__vars)
{
	...
	if ($__templater->method($__vars['field'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Adicionar campo');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Editar campo' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['field']['title'], array(array('htmlspecialchars', array()),), true));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
';
	...
	if ($__templater->method($__vars['field'], 'exists', array())) {
		$__compilerTemp1 .= '
				...
				', array(
			'label' => 'ID do campo',
			'explain' => 'Este é o identificador exclusivo para este campo. Não pode ser alterado uma vez definido.',
		)) . '
			';
	} else {
		$__compilerTemp1 .= '
	...
	if ((!$__vars['existingType']) OR ($__vars['existingType']['options'] == 'text')) {
		$__compilerTemp4 .= '
			<h3 class="block-formSectionHeader">
				<span class="collapseTrigger collapseTrigger--block" data-xf-click="toggle" data-target="< :up:next">
					<span class="block-formSectionHeader-aligner">' . 'Opções para campos de texto' . '</span>
				</span>
			</h3>
		...
		array(
			...
			'label' => 'Requisitos de correspondência de valor',
			'hint' => 'Valores vazios sempre são permitidos.',
		)) . '
	...
	<div class="block-container">
		...
		<h3 class="block-formSectionHeader">
			<span class="collapseTrigger collapseTrigger--block" data-xf-click="toggle" data-target="< :up:next">
				<span class="block-formSectionHeader-aligner">' . 'Opções gerais' . '</span>
			</span>
		</h3>
		<div class="block-body block-body--collapsible">
			' . $__compilerTemp7 . '
		...
		'label' => 'Exibição do valor HTML',
		'explain' => 'Se não for vazio, isso permite formatar o valor deste campo usando HTML, permitindo que você faça coisas como link ou marque a saída. Você pode usar esses marcadores de posição:<b>{$value}</b> - os campos exibem valor; <b>{$valueUrl}</b> - os campos exibem o valor para uso em um URL, <b>{$choice}</b> - o valor subjacente da opção escolhida, e <b>{$fieldId}</b> - o ID deste campo (<span id="FieldId">' . $__templater->escape($__vars['field']['field_id']) . '</span>).',
	)) . '
		</div>
