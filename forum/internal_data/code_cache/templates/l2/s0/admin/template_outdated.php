					...
					if ($__templater->isTraversable($__vars['outdatedForStyle'])) {
						foreach ($__vars['outdatedForStyle'] AS $__vars['outdated']) {
							...
							if ($__vars['outdated']['outdated_by_date']) {
								$__compilerTemp4 .= '
													<li>' . 'Última edição' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['outdated']['template']['last_edit_date'], array(
								))) . '</li>
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Não há templates desatualizados.' . '</div>
';
	}
