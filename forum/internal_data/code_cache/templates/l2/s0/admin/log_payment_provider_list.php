...
';
	...
	' . $__templater->button('Registros de pesquisa', array(
		...
		if ($__templater->isTraversable($__vars['entries'])) {
			foreach ($__vars['entries'] AS $__vars['entry']) {
				...
				$__compilerTemp1 .= '
					...
					'overlay' => 'true',
					'label' => '
									' . (($__vars['entry']['log_type'] == 'payment') ? 'Payment' : (($__vars['entry']['log_type'] == 'info') ? 'Informação' : (($__vars['entry']['log_type'] == 'error') ? 'Erro' : (($__vars['entry']['log_type'] == 'cancel') ? 'Cancelamento' : $__templater->escape($__vars['entry']['log_type']))))) . $__templater->escape($__vars['xf']['language']['label_separator']) . '
									<span dir="auto">' . $__templater->escape($__vars['entry']['log_message']) . '</span>
								',
					'explain' => '
									<ul class="listInline listInline--bullet">
										<li>' . ($__templater->escape($__vars['purchaseRequest']['PaymentProfile']['title']) ?: 'Perfil de pagamento desconhecido') . '</li>
