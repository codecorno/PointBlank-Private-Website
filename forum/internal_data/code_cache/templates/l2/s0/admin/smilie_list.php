...
';
	...
	<div class="buttonGroup">
		' . $__templater->button('Adicionar emoticon', array(
		'href' => $__templater->func('link', array('smilies/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
		' . $__templater->button('Adicionar categoria', array(
		...
		'title' => 'Mais opções',
			...
			<div class="menu" data-menu="menu" aria-hidden="true">
				<div class="menu-content">
					...
					<a href="' . $__templater->func('link', array('smilies/sort', ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Ordenar' . '</a>
					<a href="' . $__templater->func('link', array('smilies/import', ), true) . '" class="menu-linkRow">' . 'Importar' . '</a>
					<a href="' . $__templater->func('link', array('smilies', array(), array('export' => 1, ), ), true) . '" class="menu-linkRow">' . 'Exportar' . '</a>
				</div>
			</div>
		</div>
	</div>
...
';
	...
	if ($__templater->isTraversable($__vars['smilieData']['smilieCategories'])) {
		foreach ($__vars['smilieData']['smilieCategories'] AS $__vars['smilieCategoryId'] => $__vars['smilieCategory']) {
			...
			if ($__compilerTemp3) {
				...
				if (($__templater->func('count', array($__vars['smilieData']['smilieCategories'], ), false) > 1)) {
					$__compilerTemp6 .= '
									' . 'Ainda não foram adicionados smilies a esta categoria.' . '
								';
				} else {
					$__compilerTemp6 .= '
									' . 'Nenhum smilies foram adicionados ainda.' . '
								';
				}
				$__compilerTemp1 .= $__templater->dataRow(array(
	...
	<div class="block-container">
		...
		<h2 class="block-tabHeader tabs" role="tablist">
			...
			<a class="tabs-tab ' . ($__vars['exportView'] ? 'is-active' : '') . '" role="tab" tabindex="0" aria-controls="export-mode" href="' . $__templater->func('link', array('smilies', array(), array('export' => 1, ), ), true) . '">' . 'Exportar' . '</a>
			' . '
		</h2>
