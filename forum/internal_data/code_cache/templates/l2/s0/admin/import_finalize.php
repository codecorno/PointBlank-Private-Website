...
return array('macros' => array('notes' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['notes']['runTime'] > 3600) {
		$__compilerTemp1 .= '
			' . '' . $__templater->func('number', array($__vars['notes']['runTime'] / 3600, 2, ), true) . ' horas' . '
		';
	} else if ($__vars['notes']['runTime'] >= 120) {
		$__compilerTemp1 .= '
			' . '' . $__templater->func('number', array($__vars['notes']['runTime'] / 60, 2, ), true) . ' minutos' . '
		';
	} else {
		$__compilerTemp1 .= '
		...
		foreach ($__vars['notes']['totals'] AS $__vars['total']) {
			...
			if ($__vars['total']['total']) {
				...
				if ($__vars['total']['time'] > 3600) {
					...
					$__compilerTemp2 .= '
							' . $__templater->func('parens', array('' . $__templater->func('number', array($__vars['total']['time'] / 60, 2, ), false) . ' minutos', ), true) . '
						';
				} else {
					$__compilerTemp2 .= '
