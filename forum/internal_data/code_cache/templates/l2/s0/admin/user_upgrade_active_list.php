	...
	<div class="block">
		...
		if ($__templater->isTraversable($__vars['activeUpgrades'])) {
			foreach ($__vars['activeUpgrades'] AS $__vars['activeUpgrade']) {
				...
				array(
					'_type' => 'cell',
					'html' => ($__vars['activeUpgrade']['end_date'] ? $__templater->func('date', array($__vars['activeUpgrade']['end_date'], ), true) : 'Permanente'),
				),
				array(
					'overlay' => 'true',
		...
		$__finalCompiled .= $__templater->dataList('
			...
			'_type' => 'cell',
			'html' => '<a href="' . $__templater->func('link', array('user-upgrades/active', $__vars['userUpgrade'], array('order' => 'username', 'direction' => '', ) + $__vars['linkParams'], ), true) . '">' . 'Usuário' . '</a>',
		),
		array(
			'_type' => 'cell',
			'html' => 'Upgrade title',
		...
		array(
			'_type' => 'cell',
			'html' => '<a href="' . $__templater->func('link', array('user-upgrades/active', $__vars['userUpgrade'], array('order' => 'end_date', 'direction' => 'desc', ) + $__vars['linkParams'], ), true) . '">' . 'Data final' . '</a>',
		),
		array(
			'colspan' => '2',
			'_type' => 'cell',
