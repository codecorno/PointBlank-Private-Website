...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Excluir post');
	$__finalCompiled .= '
	...
	if ($__templater->method($__vars['post'], 'isFirstPost', array())) {
		$__compilerTemp1 .= '
				' . $__templater->formInfoRow('
					<div class="blockMessage blockMessage--important"><strong>' . 'Note' . $__vars['xf']['language']['label_separator'] . '</strong> ' . 'Este é o primeiro post no tópico. A exclusão dele excluirá todo o tópico.' . '</div>
				', array(
		)) . '
			';
	}
