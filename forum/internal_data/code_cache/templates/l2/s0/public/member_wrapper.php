...
return array('macros' => array(), 'code' => function($__templater, array $__vars)
{
	...
	if ($__vars['xf']['options']['enableMemberList']) {
		$__compilerTemp2 .= '
					<a class="blockLink ' . (($__vars['pageSelected'] == 'member_list') ? 'is-selected' : '') . '" href="' . $__templater->func('link', array('members/list', ), true) . '">' . 'Membros registados' . '</a>
				';
	}
	$__templater->modifySideNavHtml(null, '
	<div class="block">
		<div class="block-container">
			<h3 class="block-header">' . 'Membros' . '</h3>
			<div class="block-body">
				<a class="blockLink ' . (($__vars['pageSelected'] == 'overview') ? 'is-selected' : '') . '" href="' . $__templater->func('link', array('members', ), true) . '">' . 'Visão geral' . '</a>
				' . $__compilerTemp1 . '
				' . $__compilerTemp2 . '
			</div>
		</div>
...
', 'replace');
	$__finalCompiled .= '
';
	$__templater->setPageParam('sideNavTitle', 'Membros');
	$__finalCompiled .= '
