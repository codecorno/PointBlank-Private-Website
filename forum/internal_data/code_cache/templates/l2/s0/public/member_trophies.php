...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Troféus atribuídos a ' . $__templater->escape($__vars['user']['username']) . '');
	$__finalCompiled .= '
...
<div class="block">
	...
	if (!$__templater->test($__vars['trophies'], 'empty', array())) {
		...
		$__finalCompiled .= '
			<div class="block-body block-row">' . '' . $__templater->escape($__vars['user']['username']) . ' ainda não recebeu nenhum troféu.' . '</div>
		';
	}
	$__finalCompiled .= '
		<div class="block-footer block-footer--split">
			<span class="block-footer-counter">' . 'Total de pontos' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('number', array($__vars['user']['trophy_points'], ), true) . '</span>
			<span class="block-footer-controls">
				' . $__templater->button('
					' . 'Ver todos os troféus disponíveis' . '
				', array(
