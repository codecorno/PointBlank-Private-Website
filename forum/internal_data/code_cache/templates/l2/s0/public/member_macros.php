...
return array('macros' => array('moderator_menu_actions' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__templater->method($__vars['user'], 'canWarn', array())) {
		$__finalCompiled .= '
		<a href="' . $__templater->func('link', array('members/warn', $__vars['user'], ), true) . '" class="menu-linkRow">' . 'Alertar' . '</a>
	';
	}
	$__finalCompiled .= '
	';
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canViewIps', array())) {
		$__finalCompiled .= '
		<a href="' . $__templater->func('link', array('members/user-ips', $__vars['user'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Endereços de IP' . '</a>
		<a href="' . $__templater->func('link', array('members/shared-ips', $__vars['user'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'IPs compartilhados' . '</a>
	';
	}
	$__finalCompiled .= '
	';
	...
	if ($__templater->method($__vars['user'], 'canEdit', array())) {
		$__finalCompiled .= '
		<a href="' . $__templater->func('link', array('members/edit', $__vars['user'], ), true) . '" class="menu-linkRow">' . 'Editar' . '</a>
	';
	}
	$__finalCompiled .= '
	' . '
...
'member_stat_pairs' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['xf']['options']['enableTrophies']) {
		$__finalCompiled .= '
		<dl class="pairs pairs--rows pairs--rows--centered fauxBlockLink">
			<dt title="' . $__templater->filter('Pontos de troféu', array(array('for_attr', array()),), true) . '">' . 'Pontos' . '</dt>
			<dd>
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canSearch', array()) AND ($__vars['context'] != 'tooltip')) {
		$__compilerTemp2 .= '
				<div class="buttonGroup-buttonWrapper">
					' . $__templater->button('Buscar', array(
			...
			'aria-haspopup' => 'true',
					...
					<div class="menu" data-menu="menu" aria-hidden="true">
						<div class="menu-content">
							<h4 class="menu-header">' . 'Buscar conteúdo' . '</h4>
							' . '
							<a href="' . $__templater->func('link', array('search/member', null, array('user_id' => $__vars['user']['user_id'], ), ), true) . '" rel="nofollow" class="menu-linkRow">' . 'Buscar todo o conteúdo de ' . $__templater->escape($__vars['user']['username']) . '' . '</a>
							<a href="' . $__templater->func('link', array('search/member', null, array('user_id' => $__vars['user']['user_id'], 'content' => 'thread', ), ), true) . '" rel="nofollow" class="menu-linkRow">' . 'Buscar todos os tópicos de ' . $__templater->escape($__vars['user']['username']) . '' . '</a>
							' . '
						</div>
					</div>
				</div>
