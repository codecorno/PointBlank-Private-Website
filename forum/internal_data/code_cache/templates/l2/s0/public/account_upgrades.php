		...
		$__compilerTemp1 .= '
			<div class="block">
				<div class="block-container">
					<h2 class="block-header">' . 'Atualizações disponíveis' . '</h2>
			...
			foreach ($__vars['available'] AS $__vars['upgrade']) {
				...
				if (($__templater->func('count', array($__vars['upgrade']['payment_profile_ids'], ), false) > 1)) {
					...
					$__compilerTemp3 = array(array(
						'label' => $__vars['xf']['language']['parenthesis_open'] . 'Escolha um método de pagamento' . $__vars['xf']['language']['parenthesis_close'],
						'_type' => 'option',
					));
		...
		if ($__templater->isTraversable($__vars['purchased'])) {
			foreach ($__vars['purchased'] AS $__vars['upgrade']) {
				...
				if ($__vars['active']['end_date']) {
					$__compilerTemp4 .= '
										' . 'Expira' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['active']['end_date'], array(
					))) . '
									';
				} else {
					$__compilerTemp4 .= '
