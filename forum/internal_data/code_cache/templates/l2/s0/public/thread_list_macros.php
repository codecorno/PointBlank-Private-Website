...
return array('macros' => array('item' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__templater->method($__vars['thread'], 'getUserPostCount', array())) {
		$__finalCompiled .= '
			...
			'data-trigger' => 'auto',
			'title' => 'Você postou ' . $__templater->method($__vars['thread'], 'getUserPostCount', array()) . ' mensagem(ns) neste tópico',
		))) . '
				';
	}
	$__finalCompiled .= '
	...
	if (!$__vars['thread']['discussion_open']) {
		$__compilerTemp1 .= '
						<li>
							<i class="structItem-status structItem-status--locked" aria-hidden="true" title="' . $__templater->filter('Bloqueado', array(array('for_attr', array()),), true) . '"></i>
	...
	if ($__vars['thread']['discussion_type'] == 'redirect') {
		$__compilerTemp1 .= '
						<li>
							<i class="structItem-status structItem-status--redirect" aria-hidden="true" title="' . $__templater->filter('Redirecionar', array(array('for_attr', array()),), true) . '"></i>
							<span class="u-srOnly">' . 'Redirecionar' . '</span>
						</li>
					';
	} else if ($__vars['thread']['discussion_type'] == 'poll') {
		$__compilerTemp1 .= '
						<li>
							<i class="structItem-status structItem-status--poll" aria-hidden="true" title="' . $__templater->filter('Enquete', array(array('for_attr', array()),), true) . '"></i>
							<span class="u-srOnly">' . 'Enquete' . '</span>
						</li>
					';
	}
	$__compilerTemp1 .= '
	...
	if ($__vars['thread']['sticky']) {
		$__compilerTemp1 .= '
						<li>
							<i class="structItem-status structItem-status--sticky" aria-hidden="true" title="' . $__templater->filter('Destacado', array(array('for_attr', array()),), true) . '"></i>
	...
	if ($__vars['showWatched'] AND $__vars['xf']['visitor']['user_id']) {
		...
		if ($__vars['thread']['Watch'][$__vars['xf']['visitor']['user_id']]) {
			$__compilerTemp1 .= '
							<li>
								<i class="structItem-status structItem-status--watched" aria-hidden="true" title="' . $__templater->filter('Tópico seguido', array(array('for_attr', array()),), true) . '"></i>
								<span class="u-srOnly">' . 'Tópico seguido' . '</span>
							</li>
						';
	...
	$__finalCompiled .= '
			...
			<div class="contentRow-minor contentRow-minor--hideLinks">
				<ul class="listInline listInline--bullet">
					<li>' . 'Último: ' . $__templater->escape($__vars['thread']['last_post_cache']['username']) . '' . '</li>
...
'item_new_threads' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="contentRow">
		...
		<div class="contentRow-main contentRow-main--close">
			...
			<div class="contentRow-minor contentRow-minor--hideLinks">
				<ul class="listInline listInline--bullet">
					<li>' . 'Iniciado por ' . $__templater->escape($__vars['thread']['username']) . '' . '</li>
					<li>' . $__templater->func('date_dynamic', array($__vars['thread']['post_date'], array(
	))) . '</li>
					<li>' . 'Respostas' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['thread']['reply_count'], array(array('number_short', array()),), true) . '</li>
				</ul>
			</div>
			<div class="contentRow-minor contentRow-minor--hideLinks">
				<a href="' . $__templater->func('link', array('forums', $__vars['thread']['Forum'], ), true) . '">' . $__templater->escape($__vars['thread']['Forum']['title']) . '</a>
	...
	if ($__templater->method($__vars['forum'], 'canCreateThread', array())) {
		...
		' . $__templater->form('
			...
			'label' => 'Título',
				...
				<div class="js-quickThreadFields inserter-container is-hidden"><!--' . 'Carregando' . $__vars['xf']['language']['ellipsis'] . '--></div>
			</div>
		', array(
